...
export default function TrendingPage() {
  ...
  React.useEffect(() => {
    ...
    params.set("page", "1");
    router.replace(`/trending?${params.toString()}`);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [sort, status]);

  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["list", "trending", page, sort, status.join(",")],
    queryFn: async () => {
      const sParam = sort ? `&sort=${encodeURIComponent(sort)}` : "";
      const stParam = status.length ? `&status=${encodeURIComponent(status.join(","))}` : "";
      const res = await fetch(`/api/v1/list/trending?page=${page}&limit=${LIMIT}${sParam}${stParam}`);
